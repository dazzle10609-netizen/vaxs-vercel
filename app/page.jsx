...
const products = [
  {
    icon: "chart",
    title: "Spot Trading",
    description: "High-speed order execution with deep market visibility for major digital asset pairs."
  },
  {
    icon: "layers",
    title: "Smart Contract Trading",
    description: "Contract-based trading infrastructure designed for scalable risk control and real-time settlement states."
  },
  {
    icon: "wallet",
    title: "Wallet Management",
    description: "Unified Web3 wallet accounts, asset balances, deposits, withdrawals and internal ledger synchronization."
  },
  {
    icon: "radio",
    title: "Market Data Services",
    description: "Live prices, market depth, trading history and WebSocket streaming for trading applications."
  }
];
...
export function runVAXSHomepageTests() {
  const results = [
    products.length === 4,
    markets.includes("Southeast Asia") && markets.includes("Japan") && markets.includes("Korea"),
    stats.length === 4,
    chartBars.every((height) => height > 0 && height <= 100),
    products.every((product) => product.title.length > 0 && product.description.length > 0 && product.icon.length > 0)
  ];

  return results.every(Boolean);
}

export default function VAXSHomepage() {
  return (
    <div className="min-h-screen overflow-hidden bg-[#050716] text-white">
      ...
      <header className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
        <a href="#top" className="flex items-center gap-3" aria-label="VAXS homepage">
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white font-black text-[#2628b4] shadow-lg shadow-[#2628b4]/30">
            V
          </div>
          <div>
        ...
        <button type="button" className="rounded-full border border-white/15 bg-white/10 px-5 py-2 text-sm backdrop-blur transition hover:bg-white/15">
          Launch App
        </button>
      </header>
        ...
        <section className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-6 pb-20 pt-16 lg:grid-cols-2 lg:pt-24">
          <div>
            ...
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.15 }} className="mt-9 flex flex-col gap-4 sm:flex-row">
              ...
              <button type="button" className="rounded-full border border-white/15 bg-white/5 px-7 py-4 font-medium text-white backdrop-blur transition hover:bg-white/10">
                Explore API
              </button>
            </motion.div>
          </div>
              ...
              <div className="rounded-[1.5rem] border border-white/10 bg-[#070a1f]/90 p-5">
                ...
                <div className="mt-5 grid grid-cols-3 gap-3 text-sm">
                  <div className="rounded-2xl bg-white/[0.04] p-4">
                    ...
                    <div className="mt-1 text-emerald-300">68,421.8</div>
                  </div>
                  <div className="rounded-2xl bg-white/[0.04] p-4">
                    <div className="text-slate-500">Ask</div>
                    <div className="mt-1 text-rose-300">68,435.2</div>
                  </div>
                  <div className="rounded-2xl bg-white/[0.04] p-4">
                    <div className="text-slate-500">Vol</div>
                    <div className="mt-1">1.28B</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {products.map((product) => (
              <article key={product.title} className="group rounded-3xl border border-white/10 bg-white/[0.05] p-6 backdrop-blur transition hover:-translate-y-1 hover:bg-white/[0.08]">
                <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#2628b4]/30 text-cyan-200">
          ...
          <div className="grid items-center gap-8 lg:grid-cols-2">
            <div>
              ...
              <p className="mt-6 text-lg leading-8 text-slate-300">
                VAXS uses a hybrid exchange model. High-frequency order matching, clearing and state synchronization are completed within the internal ledger system, while asset settlement is connected to on-chain verification and Web3 wallet account management.
              </p>
            </div>
              ...
              <div className="grid gap-4">
                <div className="rounded-3xl bg-[#2628b4]/20 p-6">
                  <div className="flex items-center gap-3 text-xl font-semibold">
                    <Icon name="zap" className="h-6 w-6 shrink-0 text-cyan-300" />
                    Off-chain Centralized Matching
                  </div>
                ...
                <div className="rounded-3xl bg-white/[0.05] p-6">
                  <div className="flex items-center gap-3 text-xl font-semibold">
                    <Icon name="lock" className="h-6 w-6 shrink-0 text-cyan-300" />
        ...
        <section id="markets" className="mx-auto max-w-7xl px-6 py-20">
          <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/[0.08] to-white/[0.03] p-8 backdrop-blur md:p-12">
            <div className="grid gap-10 lg:grid-cols-[1fr_1.2fr]">
              <div>
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#2628b4]/30 text-cyan-200">
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {markets.map((market) => (
                  <div key={market} className="flex min-h-40 items-end rounded-3xl border border-white/10 bg-[#070a1f]/70 p-6">
                    <div>
                      <div className="mb-3 h-2 w-12 rounded-full bg-cyan-300" />
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-8 backdrop-blur md:p-12 lg:col-span-2">
              ...
              <p className="mt-5 max-w-3xl leading-8 text-slate-300">
                Connect to real-time market data, trading endpoints, wallet operations and account services through API-first infrastructure built for fast integration.
              </p>
              <button type="button" className="mt-8 rounded-full bg-white px-7 py-4 font-medium text-[#080b1f]">
                View Documentation
              </button>
            </div>
            ...
            <div className="rounded-[2rem] border border-white/10 bg-[#2628b4]/20 p-8 backdrop-blur">
              ...
              <p className="mt-4 leading-7 text-slate-300">
                Account protection, asset state control, permission management and operational monitoring are built into the platform foundation.
              </p>
            </div>
          </div>
        </section>
            ...
            <div className="grid items-center gap-8 md:grid-cols-[1.4fr_0.6fr]">
              <div>
                ...
                <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">
                  Access trading, wallet and market data services through a digital asset exchange platform designed for Asia-focused growth.
                </p>
              </div>
              <button type="button" className="rounded-full bg-[#2628b4] px-8 py-4 font-medium text-white shadow-xl shadow-[#2628b4]/25">
                Contact Sales
              </button>
            </div>
          </div>
